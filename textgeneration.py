...
def collate_fn(batch):
    ...
    encoded_data = tokenizer(
      sequences, padding=True, truncation=True, max_length=64
    )
...
train_dataloader = DataLoader(product_reviews_train, batch_size=4, collate_fn=collate_fn)
valid_dataloader = DataLoader(product_reviews_valid, batch_size=4, collate_fn=collate_fn)
test_dataloader = DataLoader(product_reviews_test, batch_size=4, collate_fn=collate_fn)
...
def run(model, optim):
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            labels = torch.eye(max_value+1)[labels].to(torch.long).to(device)
